...
$feed = Mojo::Feed->new($file);
isa_ok( $feed, 'Mojo::Feed' );
is( $feed->title, 'First Weblog', 'title ok' );

# parse a string
my $str = Mojo::File->new($file)->slurp;
$feed = Mojo::Feed->new($str);
isa_ok( $feed, 'Mojo::Feed' );
is( $feed->title, 'First Weblog', 'title ok' );

# parse a URL
$feed =
  Mojo::Feed->new->ua( $t->app->ua )->parse( Mojo::URL->new("/atom.xml") );
isa_ok( $feed, 'Mojo::Feed' );
is( $feed->title, 'First Weblog', 'title ok' );
...
for my $file ( sort keys %Feeds ) {
    ...
    my $feed = Mojo::Feed->new($path) or die "parse feed returned undef";
...
$feed = Mojo::Feed->new( File::Spec->catdir( $sample_dir, 'rss20.xml' ) )
  or die "parse fail";
...
$feed =
  Mojo::Feed->new( File::Spec->catdir( $sample_dir, 'rss20-no-summary.xml' ) )
  or die "parse fail";
$entry = $feed->items->[0];
...
$feed =
  Mojo::Feed->new( File::Spec->catdir( $sample_dir, 'rss10-invalid-date.xml' ) )
  or die "parse fail";
$entry = $feed->items->[0];
...
$feed =
  Mojo::Feed->new( File::Spec->catdir( $sample_dir, 'itunes_summary.xml' ) )
  or die "parse failed";
$entry = $feed->items->[0];
...
$feed = Mojo::Feed->new( $t->app->ua->get('/link1.html')->res->body );
...
my $feed_from_file =
  Mojo::Feed->new( File::Spec->catdir( $sample_dir, 'plasmastrum.xml' ) );
my $tx            = $t->get_ok('/plasmastrum.xml')->tx;
my $feed_from_tx  = Mojo::Feed->new->ua( $t->app->ua )->parse( $tx->res->body );
my $feed_from_url = Mojo::Feed->new->ua( $t->app->ua )
  ->parse( Mojo::URL->new('/plasmastrum.xml') );
my $feed_from_url2 =
  Mojo::Feed->new->ua( $t->app->ua )->parse( Mojo::URL->new('/plasm') );
