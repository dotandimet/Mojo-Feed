...
# parse a URL
$feed =
  Mojo::Feed->new( Mojo::URL->new("/atom.xml") );
isa_ok( $feed, 'Mojo::Feed' );
is( $feed->title, 'First Weblog', 'title ok' );
...
$feed =
  Mojo::Feed->new( Mojo::URL->new("/feed") );
...
$feed = Mojo::Feed->new( path( $sample_dir, 'rss20.xml' ) )
  or die "parse fail";
...
$feed = Mojo::Feed->new( path( $sample_dir, 'rss20-no-summary.xml' ) )
  or die "parse fail";
$entry = $feed->items->[0];
ok( $entry->summary eq $entry->content, 'no summary use content/description' );
like( $entry->content, qr/<p>This is a test.<\/p>/ );

$feed = Mojo::Feed->new( path( $sample_dir, 'rss10-invalid-date.xml' ) )
  or die "parse fail";
$entry = $feed->items->[0];
...
$feed = Mojo::Feed->new( path( $sample_dir, 'itunes_summary.xml' ) )
  or die "parse failed";
$entry = $feed->items->[0];
...
$feed = Mojo::Feed->new( path($sample_dir, 'itunes_author_order.xml') )
        or die "parse failed";
...
# Invalid input:
$feed = $Mojo::Feed->new("<xml><garbage>this is invalid</garbage></xml>");
...
my $feed_from_file =
  $Mojo::Feed->new( path( $sample_dir, 'plasmastrum.xml' ) );
my $tx            = $t->get_ok('/plasmastrum.xml')->tx;
my $feed_from_tx  = Mojo::Feed->new( $t->app->ua )->parse( $tx->res->body );
my $feed_from_url = Mojo::Feed->new( $t->app->ua )
  ->parse( Mojo::URL->new('/plasmastrum.xml') );
my $feed_from_url2 =
  Mojo::Feed->new( $t->app->ua )->parse( Mojo::URL->new('/plasm') );
